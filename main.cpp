		...
		if (printFound != string::npos) {
			...
			for (int i = 0; i < line.size(); i++) {
				...
				if (i == (line.size() - 1) && assignmentLine) {
					//cout << "here is the temp file: " << temp << endl;
					if (inFunction) {
						//cout << "inFunction entered" << endl;
						
						addWhiteSpace(temp);
						//cout << "here is the temp file after whitespace:" << temp << endl;
						for (int x = 0; x<functionArrayIndex; x++) {
							string toFind = " " + functionVariableNames[x] + " ";
							size_t found = temp.find(toFind);
							if (found != string::npos) {
								//cout << "a function has been found in the line:" << temp << " of name: " << functionVariableNames[x] << endl;
								//temp.replace(found, toFind.size(), to_string(functionVariableValues[x]));
								ReplaceAll(temp, toFind, to_string(functionVariableValues[x]));
								//cout << "function after replace" << temp << endl;
							}
						}

					}
					else {
						//cout << "else function entered" << endl;
